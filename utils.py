...
def cal_dice(output, target):
    ...
    dice1 = Dice((output == 3).float(), (target == 3).float())#增强肿瘤区域ET
    dice2 = Dice(((output == 1) | (output == 3)).float(), ((target == 1) | (target == 3)).float())#肿瘤核心区域TC
    dice3 = Dice((output != 0).float(), (target != 0).float())#整个肿瘤WT
